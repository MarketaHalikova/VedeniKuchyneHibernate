...
public class UpravaSurovinyController extends Observable {
	 ...
	 *            - vybraná surovina
	 */
	void nactiHodnoty(Kuchyne kuchyne, String vybrana) {
	...
	/**
	 * Metoda vypíše položky zadné suroviny (název, množství a jednotka) do
	 * příslušných polí
	 */
	private void vypisSurovinu() {
		...
		/*
		 * String jedn =
		 * kuchyne.getAktualniSklad().najdiSurovinu(vybrana).getJednotka().name(
		 * ); switch(jedn) { case "kg": jednotka.getSelectionModel().select(0);
		 * break; case "ks": jednotka.getSelectionModel().select(1); break; case
		 * "l": jednotka.getSelectionModel().select(2); }
		 */
	}

	/**
	 * Metoda pro akci spuštěnou talčítkem "Uložit". Surovina se změní podle
	 * zadaných kritérií a přepíše se v seznamu surovin na skladě. Při chybějích
	 * nebo mylných udajích se zobrazí alert s nápovědou.
	 * 
	 * @param event
	 */
